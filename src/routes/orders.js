...
class OrdersRoute {
	constructor(router,databaseConnection) {
		this.router = router;
		this.registerRoutes();
		OrdersService.db=databaseConnection;
		OrderAddressService.db=databaseConnection;
		OrderItemsService.db=databaseConnection;
		OrderTransactionsService.db=databaseConnection;
		OrdertDiscountsService.db=databaseConnection;
	...
	async addTransaction(req, res, next) {
		try {
			let data = await OrderTransactionsService.addTransaction(req.params.id, req.body);
			return res.send(data);
		} catch(err) {
			return next(err);
		}
	...
	async updateTransaction(req, res, next) {
		try {
			...
			let data = await OrderTransactionsService.updateTransaction(
				order_id,
				transaction_id,
				req.body
			);
	...
	async deleteTransaction(req, res, next) {
		try {
			const order_id = req.params.id;
			const transaction_id = req.params.item_id;
			let data = await OrderTransactionsService.deleteTransaction(order_id, transaction_id);
			return res.send(data);
		} catch(err) {
			return next(err);
		}
