...
class CustomersService {
	...
	getCustomers(params = {}) {
		...
		return Promise.all([
			...
			this.db
				...
				.limit(limit)
				.toArray(),
			this.db.collection('customers').countDocuments(filter)
	...
	async updateCustomer(id, data) {
		...
		// is email unique
		if (customer.email && customer.email.length > 0) {
			const customerCount = await this.db.collection('customers').count({
				_id: {
					$ne: customerObjectID
				},
	...
	addAddress(customer_id, address) {
		...
		return this.db.collection('customers').updateOne(
			{
				_id: customerObjectID
			},
			{
