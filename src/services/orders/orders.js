...
class OrdersService {
	...
	getOrders(params) {
		...
		OrderStatusesService.db=this.db;
		ShippingMethodsLightService.db=this.db;
		PaymentMethodsLightService.db=this.db;
		...
		return Promise.all([
			this.db
				...
				.limit(limit)
				.toArray(),
			this.db.collection('orders').countDocuments(filter),
	...
	async addOrder(data) {
		...
		const insertResponse = await this.db.collection('orders').insertMany([order]);
